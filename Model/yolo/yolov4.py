...
class YOLOv4(nn.Module):
    ...
    def postprocess(self, bboxes, scores):
        ...
        cls_inds = torch.from_numpy(cls_inds).unsqueeze(-1)
        # print(bboxes.shape)
        # print(scores.shape)
        # print(cls_inds.shape)

        batch_info = torch.cat([bboxes, scores, cls_inds], dim=-1)
        return batch_info


    @torch.no_grad()
    def inference_images(self, x):
        KA = self.num_anchors
        C = self.num_classes
        # backbone
        ...
        batches_info = []
        for id_img in range(x.shape[0]):
            # det
            pred_s = self.head_det_1(c13)[id_img]
            pred_m = self.head_det_2(c16)[id_img]
            pred_l = self.head_det_3(c19)[id_img]
            ...
            # post-process
            batch_info = self.postprocess(bboxes, scores)
    ...
    def forward(self, x, targets=None):
        if targets is None:
            return self.inference_images(x)
        else:
            B = x.size(0)
            KA = self.num_anchors
            C = self.num_classes
